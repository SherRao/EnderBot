...
public class UnmuteCommand extends ChatCommandProcessor {

	public UnmuteCommand( Client client, String command, Permission permission, String usage, String... aliases ) {
		...
		if( sender.hasPermission( channel, Permission.VOICE_MUTE_OTHERS ) ) {
			if( args.length > 0 ) {
				...
				if( member.getVoiceState().isMuted() ) {
					server.getController().setMute( member, false).complete();
					channel.sendMessage( ":trophy: " + sender.getAsMention() + " has unmuted " + member.getAsMention() ).complete();
					
				} else
					channel.sendMessage( ":negative_squared_cross_mark: That user is already unmuted!" ).complete();
