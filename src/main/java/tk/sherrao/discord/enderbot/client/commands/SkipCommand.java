...
public class SkipCommand extends ChatCommandProcessor {

	protected AudioIO player;
	
	public SkipCommand( Client client, String command, Permission permission, String usage, String... aliases ) {
	...
	public  void execute( User sender, Guild server, TextChannel channel, String alias,  String[] args ) {
		...
		if( channel.getName().equals( musicChannel ) ) {	
			...
			if( member.getVoiceState().inVoiceChannel() )
				if( member.getVoiceState().inVoiceChannel() )
					player.skip( sender, server, member.getVoiceState().getChannel(), channel );
